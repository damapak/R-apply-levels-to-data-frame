...
for (x in names(survey_data)) {
  ...
  if ( is.numeric(survey_data[,x]) == FALSE && 
    ...
    for (y in names(level_check)) { #for all pre-defined level names
      ...
      if (all(levels(survey_data[,x])[! levels(survey_data[,x]) %in% c(""," ","NA","NOT APPLICABLE") ] %in% level_check[[y]] ==TRUE)) {
        ...
        #convert field to new ordered factor
        #survey_data[,x] = factor(survey_data[,x], level_check[[y]], ordered=TRUE)
      }
    }
    if (length(matched_levels) > 1 ) {
      #for multiple level matches.  ultimately allow manual selection for multiple matches.
      survey_data[,x] = factor(survey_data[,x], level_check[[matched_levels[[1]]]], ordered=TRUE)
    } else if (length(matched_levels) == 1) {
      #for single level matches (one pre-defined level that fits criteria)
      survey_data[,x] = factor(survey_data[,x], level_check[[matched_levels[[1]]]], ordered=TRUE)
    } else {
